...
BEGIN_MESSAGE_MAP(CPersistentFrame, CTabSDIFrameWnd)
...
void CPersistentFrame::ActivateFrame(int nCmdShow)
{
    ...
    CTabSDIFrameWnd::ActivateFrame(nCmdShow);
}



void CPersistentFrame::DockControlBarLeftOf(CControlBar* Bar, CControlBar* ObjRef)
{
	CRect rect;
	DWORD dw;
	UINT n;

	RecalcLayout(TRUE);  //重新显示

	ObjRef->GetWindowRect(&rect);
	rect.OffsetRect(-1,0);			//设置偏移植以停靠在同一边上
	dw = ObjRef->GetBarStyle();
	n = 0;
...
void CPersistentFrame::DockControlBarRigthOf(CControlBar* Bar, CControlBar* ObjRef)
{
	CRect rect;
	DWORD dw;
	UINT n;
...
void CPersistentFrame::DockControlBarTopOf(CControlBar* Bar, CControlBar* ObjRef)
{
	CRect rect;
	DWORD dw;
	UINT n;
	...
	ObjRef->GetWindowRect(&rect);
	rect.OffsetRect(0, -1);			//设置偏移植以停靠在同一边上
	dw = ObjRef->GetBarStyle();
	n = 0;
	n = (dw&CBRS_ALIGN_TOP) ? AFX_IDW_DOCKBAR_TOP : n;
	n = (dw&CBRS_ALIGN_BOTTOM && n==0) ? AFX_IDW_DOCKBAR_BOTTOM : n;
...
void CPersistentFrame::DockControlBarBottomOf(CControlBar* Bar, CControlBar* ObjRef)
{
	CRect rect;
	DWORD dw;
	UINT n;
