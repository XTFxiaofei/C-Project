...
IMPLEMENT_DYNCREATE(CTabSDIFrameWnd, CNewFrameWnd)
...
BEGIN_MESSAGE_MAP(CTabSDIFrameWnd, CNewFrameWnd)
...
BOOL CTabSDIFrameWnd::DeleteActiveView()
{
	...
	CMTASDoc* pDoc = (CMTASDoc*)GetActiveDocument();
//	CString sTitle = pDoc->GetTitle();
	...
	for(int i=0; i<m_wndTab.GetItemCount(); i++)
	{
		...
		if(dwData==(DWORD)pViewRemove)
		{
			...
			CMTASView* pMTASView = dynamic_cast<CMTASView*>(pView);
			CMyView* pMyView = dynamic_cast<CMyView*>(pView);
			if(pMTASView) {
				pDoc->SetTitle(pMTASView->m_sLabel);		
			}
			else if(pMyView)
				pDoc->SetTitle(pMyView->GetLabel());
...
//		CString sTitle = pDoc->GetTitle();
		...
		CView* pViewRemove = GetActiveView();
		((CMyView*)pView)->SwitchToThisCntrItem();
