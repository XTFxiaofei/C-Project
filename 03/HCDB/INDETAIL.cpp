// INDETAIL.cpp : implementation file
//
...
CINDETAIL::CINDETAIL(CWnd* pParent /*=NULL*/)
	: CDialog(CINDETAIL::IDD, pParent)
{
	//{{AFX_DATA_INIT(CINDETAIL)
	m_begintime = 0;
	m_endtime = 0;
	m_edit2 = _T("");
	//}}AFX_DATA_INIT
...
void CINDETAIL::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CINDETAIL)
...
void CINDETAIL::GetMercRecord(CString sqlstr)
{
	 m_mercRecord=NULL;
...
void CINDETAIL::GetRecord(CString sqlstr)
{
	 m_record=NULL;
...
void CINDETAIL::Connect()
{
	//////////////////////////////////////conection

}
BEGIN_MESSAGE_MAP(CINDETAIL, CDialog)
	//{{AFX_MSG_MAP(CINDETAIL)
...
/////////////////////////////////////////////////////////////////////////////
// CINDETAIL message handlers

BOOL CINDETAIL::OnInitDialog() 
{
...
void CINDETAIL::OnItemchangedList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	a.discount,b.storid,b.stortype,b.storinfo from tb_merchangiseinfo a inner join tb_storage b on a.id='%s' and \
		b.merchandiseid='%s' and storflag='1' inner join tb_provider c on a.provid=c.provid",m_mercid,m_mercid);
	GetMerc(sqlstr);
	*pResult = 0;
}

void CINDETAIL::OnCheck1() 
{
	if(!m_check.GetCheck())
	{
		m_edit2="";
...
void CINDETAIL::OnCheck2() 
{
	if(!m_check2.GetCheck())
   	InitDate();
}

CString CINDETAIL::GetMerc(CString sqlstr)
{
	...
	m_list2ctrl.ResetContent();
	UpdateData(false);
	m_mercRecord->Open((_variant_t )sqlstr,theApp.m_connection.GetInterfacePtr(),
...
int CINDETAIL::GetRecordCount(_RecordsetPtr ptr)
{
   	int count=0;
	if(ptr!=NULL)
	{
