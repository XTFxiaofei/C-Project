...
void CSCBPushPinButton::Paint(CDC* pDC)
{
    ...
    if (bPushed) {    
        ...
        CBrushDC brush(*pDC, HLS_TRANSFORM (crHighLight, +50, -50));
		pDC->Rectangle (&rc);
	}
    else if (bRaised) {        
		COLORREF crHighLight = ::GetSysColor (COLOR_HIGHLIGHT);
        CPenDC pen(*pDC, crHighLight);
        CBrushDC brush(*pDC, HLS_TRANSFORM (crHighLight, +70, -57));
        pDC->Rectangle (&rc);
	}
    ...
    pDC->TextOut(ptOrg.x + 3, ptOrg.y + 3, CString(_T("v"))); // < >-like
...
void CPinDockBar::NcCalcClient(LPRECT pRc, UINT nDockBarID)
{
    ...
    if (bHorz)
        ...
        ptOrgBtn = CPoint(rc.right - 15, rc.top - 14);

    m_biHide.Move(ptOrgBtn - rcBar.TopLeft());
	if(!IsFloating())
	{
	    if (bHorz)
		    m_PushPin.Move(CPoint(rc.left - 12, rc.bottom - 14) - rcBar.TopLeft());
	    else
			m_PushPin.Move(CPoint(rc.left , rc.top - 14) - rcBar.TopLeft());
	}
    *pRc = rc;
}
...
UINT CPinDockBar::OnNcHitTest(CPoint point) 
{
