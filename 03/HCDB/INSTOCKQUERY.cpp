// INSTOCKQUERY.cpp : implementation file
//
...
CINSTOCKQUERY::CINSTOCKQUERY(CWnd* pParent /*=NULL*/)
	: CDialog(CINSTOCKQUERY::IDD, pParent)
{
	//{{AFX_DATA_INIT(CINSTOCKQUERY)
	m_begintime = 0;
	m_endtime = 0;
	m_edit2 = _T("");
	//}}AFX_DATA_INIT
...
void CINSTOCKQUERY::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CINSTOCKQUERY)
...
BEGIN_MESSAGE_MAP(CINSTOCKQUERY, CDialog)
	//{{AFX_MSG_MAP(CINSTOCKQUERY)
...
/////////////////////////////////////////////////////////////////////////////
// CINSTOCKQUERY message handlers

void CINSTOCKQUERY::OnButton1() 
{
		...
		if(m_check.GetCheck()==false&&(m_check2.GetCheck()==true))
	{
		UpdateData();
        CString sqlstr;
        sqlstr.Format("select * from tb_storage where stordate between '%s' and '%s' and storflag='1'",m_begintime.Format("%Y-%m-%d"),m_endtime.Format("%Y-%m-%d"));
        GetRecord(sqlstr);
        int row=0;
		m_list.DeleteAllItems();
		while(!m_record->_EOF)
...
void CINSTOCKQUERY::OnKillfocusEdit2() 
{
	// TODO: Add your control notification handler code here
    //	MessageBox("");
}

void CINSTOCKQUERY::SetListStyle()
{
...
void CINSTOCKQUERY::GetRecord(CString sqlstr)
{
     m_record=NULL;
...
void CINSTOCKQUERY::GetMercRecord(CString sqlstr)
{
     m_mercRecord=NULL;
