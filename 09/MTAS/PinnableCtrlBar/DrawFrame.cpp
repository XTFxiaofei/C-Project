...
void CDrawFrame::OnPaint() 
{
	...
	CRect rt;
	GetClientRect(rt);
	dc.Rectangle(rt.left, rt.top, rt.Width(), rt.Height());
...
void CDrawFrame::OnNcPaint() 
{
	...
	rc.bottom = rc.top + m_TitleHeight - offset;
	...
	CRect rcTmp = rc;
	rcTmp.bottom += offset;
	dc.FillSolidRect(&rcTmp, ::GetSysColor(COLOR_BTNFACE));
...
void CDrawFrame::OnNcMouseMove(UINT nHitTest, CPoint point) 
{
	...
	BOOL bPushPin = (nHitTest == HTOBJECT);

	BOOL bWasPushed = m_PushPin.bPushed;
    m_PushPin.bPushed = bPushPin && bLButtonDown;

    BOOL bWasRaised = m_PushPin.bRaised;
    m_PushPin.bRaised = bPushPin;

    if(bWasPushed != m_PushPin.bPushed || bWasRaised != m_PushPin.bRaised)
		bNeedPaint = TRUE;
