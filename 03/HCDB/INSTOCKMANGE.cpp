// INSTOCKMANGE.cpp : implementation file
//
...
CINSTOCKMANGE::CINSTOCKMANGE(CWnd* pParent /*=NULL*/)
	: CDialog(CINSTOCKMANGE::IDD, pParent)
{
	//{{AFX_DATA_INIT(CINSTOCKMANGE)
	m_edit1 = _T("");
	m_begin = 0;
	m_end = 0;
	m_allnum = _T("");
...
void CINSTOCKMANGE::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CINSTOCKMANGE, CDialog)
	//{{AFX_MSG_MAP(CINSTOCKMANGE)
...
void CINSTOCKMANGE::OnButton6() 
{
	...
	sqlstr.Format("select a.merchandiseid,b.mercname,a.unitprice,a.numbers,b.discount,a.paymoney,a.operid,a.storid,a.storflag from tb_storage a inner join tb_merchangiseinfo b\
		on a.merchandiseid=b.id where a.merchandiseid='%s' and a.storflag='1'",m_edit1);
   ...
   if(str=="入库编号汇总")
	{
	sqlstr.Format("select a.merchandiseid,b.mercname,a.unitprice,a.numbers,b.discount,a.paymoney,a.operid,a.storid,a.storflag from tb_storage a inner join tb_merchangiseinfo b\
		on a.merchandiseid=b.id where a.storid='%s' and a.storflag='1'",m_edit1);
	sqlstr1.Format("select sum(numbers) as allnum from tb_storage a inner join tb_merchangiseinfo b\
		on a.merchandiseid=b.id where a.storid='%s' and a.storflag='1'",m_edit1);
	sqlstr2.Format("select sum(paymoney) as allmoney from tb_storage a inner join tb_merchangiseinfo b\
		on a.merchandiseid=b.id where a.storid='%s' and a.storflag='1'",m_edit1);
	...
	sqlstr.Format("select a.merchandiseid,b.mercname,a.unitprice,a.numbers,b.discount,a.paymoney,a.operid,a.storid,a.storflag from tb_storage a inner join tb_merchangiseinfo b\
		on a.merchandiseid=b.id where a.stordate between '%s' and '%s' and a.storflag='1'",tm1.Format("%Y-%m-%d"),tm2.Format("%Y-%m-%d"));
	sqlstr1.Format("select sum(numbers) as allnum from tb_storage a inner join tb_merchangiseinfo b\
		on a.merchandiseid=b.id where a.stordate between '%s' and '%s' and a.storflag='1'",tm1.Format("%Y-%m-%d"),tm2.Format("%Y-%m-%d"));
	sqlstr2.Format("select sum(paymoney) as allmoney from tb_storage a inner join tb_merchangiseinfo b\
		on a.merchandiseid=b.id where a.stordate between '%s' and '%s' and a.storflag='1'",tm1.Format("%Y-%m-%d"),tm2.Format("%Y-%m-%d"));
...
void CINSTOCKMANGE::SetListstyle()
{
	...
	m_list.InsertColumn(7,"入库编号",LVCFMT_LEFT,120);
	m_list.SetExtendedStyle(LVS_EX_FULLROWSELECT|LVS_EX_GRIDLINES);

}

BOOL CINSTOCKMANGE::OnInitDialog() 
{
	...
	Sum("select a.merchandiseid,b.mercname,a.unitprice,a.numbers,b.discount,a.paymoney,a.operid,a.storid ,a.storflag from tb_storage a inner join tb_merchangiseinfo b\
		 on a.merchandiseid=b.id where a.storflag='1' ");
...
void CINSTOCKMANGE::SetList2Style()
{
	...
	m_list2.InsertColumn(6,"操作员",LVCFMT_LEFT,120);
	m_list2.InsertColumn(7,"入库编号",LVCFMT_LEFT,120);
	m_list2.SetExtendedStyle(LVS_EX_FULLROWSELECT|LVS_EX_GRIDLINES);

}
void CINSTOCKMANGE::ShowWindows(bool flag)
{
...
void CINSTOCKMANGE::OnCloseupCombo1() 
{
...
void CINSTOCKMANGE::Sum(CString sqlstr)
{
    if(m_record!=NULL)
	{
		m_record=NULL;
...
void CINSTOCKMANGE::ShowList2(CString sqlstr)
{
	m_record->Close();
	m_record->Open(_bstr_t(sqlstr),theApp.m_connection.GetInterfacePtr(),
...
void CINSTOCKMANGE::GetAllNum(CString sqlstr)
{  
    m_record->Close();
	m_record->Open(_bstr_t(sqlstr),theApp.m_connection.GetInterfacePtr(),adOpenDynamic,adLockOptimistic,adCmdText);
	if(GetRecordCount(m_record))
...
void CINSTOCKMANGE::GetAllMoney(CString sqlstr)
{
	m_record->Close();
	m_record->Open(_bstr_t(sqlstr),theApp.m_connection.GetInterfacePtr(),adOpenDynamic,adLockOptimistic,adCmdText);
	if(GetRecordCount(m_record))
...
int CINSTOCKMANGE::GetRecordCount(_RecordsetPtr ptr)
{
   int count=0;
	if(ptr!=NULL)
	{
